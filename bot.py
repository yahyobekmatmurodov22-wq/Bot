"""
KINOCHI BOT — SINGLE FILE
Original project combined into one bot.py for easy GitHub/Pydroid deployment.

IMPORTANT:
Set BOT_TOKEN and ADMINS through environment variables / .env.
Do NOT commit a real BotFather token to GitHub.
"""
...
# └──────────────────────────────────────────────────────────┘
_BOT_TOKEN_DEFAULT = "PUT_YOUR_BOT_TOKEN_HERE"      # @BotFather'dan olgan token
_ADMINS_DEFAULT = "PUT_YOUR_TELEGRAM_ID_HERE"       # @userinfobot'dan ID'ingiz (bir nechta bo'lsa vergul bilan: 111,222)
# ┌──────────────────────────────────────────────────────────┐
# │  👆 SHU IKKI QATORNI O'ZINGIZNIKIGA ALMASHTIRING 👆        │
# └──────────────────────────────────────────────────────────┘

# Render'da Environment Variables qo'shsangiz, ular yuqoridagi
# qiymatlardan ustun turadi (kodni ochib o'zgartirish shart bo'lmaydi).
BOT_TOKEN: str = os.getenv("BOT_TOKEN", _BOT_TOKEN_DEFAULT).strip()

ADMINS: list[int] = [
    int(admin_id)
    for admin_id in os.getenv("ADMINS", _ADMINS_DEFAULT).replace(" ", "").split(",")
    if admin_id.isdigit()
]

# Baza oddiy fayl ko'rinishida saqlanadi
DB_PATH: str = os.getenv("DB_PATH", "kinochi.db")
...
if not BOT_TOKEN or BOT_TOKEN == "PUT_YOUR_BOT_TOKEN_HERE":
    raise RuntimeError(
        "BOT_TOKEN kiritilmagan! bot.py faylining tepasida "
        "_BOT_TOKEN_DEFAULT qatoriga @BotFather'dan olgan tokenni yozing."
    )


# ============================================================
# kinochi_bot/states.py
# ============================================================
...
# ============================================================
# kinochi_bot/handlers/search.py
# ============================================================
...
@router_search.message(F.text & ~F.text.startswith("/"))
...
@router_search.callback_query(F.data.startswith("get:"))
...
router_admin = Router()
# Butun router_admin faqat adminlar uchun ishlaydi
router_admin.message.filter(F.from_user.id.in_(ADMINS))
router_admin.callback_query.filter(F.from_user.id.in_(ADMINS))
...
@router_admin.message(F.text == BTN_ADMIN)
...
@router_admin.message(F.text == BTN_BACK)
...
@router_admin.message(F.text == BTN_CANCEL)
...
@router_admin.message(F.text == BTN_ADD_MOVIE)
...
@router_admin.message(AddMovie.video, F.video | F.document | F.animation)
...
@router_admin.message(AddMovie.video)
...
@router_admin.message(AddMovie.field, F.text)
...
@router_admin.message(AddMovie.code)
...
@router_admin.message(F.text == BTN_STATS)
...
@router_admin.message(F.text == BTN_MOVIES)
...
@router_admin.message(FindMovie.query, F.text, ~F.text.in_(MENU_BUTTONS))
...
@router_admin.callback_query(F.data.startswith("page:"))
...
@router_admin.callback_query(F.data.startswith("movie:"))
...
@router_admin.callback_query(F.data.startswith("del:"))
...
@router_admin.callback_query(F.data.startswith("delyes:"))
...
@router_admin.callback_query(F.data.startswith("edit:"))
...
@router_admin.callback_query(F.data.startswith("field:"))
...
@router_admin.message(EditMovie.value, F.text)
...
@router_admin.message(EditMovie.value)
...
@router_admin.message(F.text == BTN_TEMPLATE)
...
@router_admin.callback_query(F.data == "tpl_edit")
...
@router_admin.message(EditTemplate.value, F.text)
...
@router_admin.message(F.text == BTN_CHANNELS)
...
@router_admin.callback_query(F.data == "chadd")
...
@router_admin.message(AddChannel.channel, F.chat_shared)
...
@router_admin.message(AddChannel.channel)
...
@router_admin.callback_query(F.data.startswith("chdel:"))
...
# ============================================================
# kinochi_bot/middlewares/subscription.py
# ============================================================
...
def get_routers() -> tuple[Router, ...]:
    ...
    MUHIM: router_admin router_search'dan OLDIN turishi kerak.
    router_search o'zida "/" bilan boshlanmagan har qanday matnni
    qidiruv so'rovi deb ushlab qoladi — agar u oldinroq tursa,
    admin panel tugmalari (Kino qo'shish, Statistika va h.k.)
    unga yetib bormay, qidiruv sifatida yutilib ketadi.
    router_admin faqat ADMINS ro'yxatidagilar uchun ishlagani
    sababli, oddiy foydalanuvchilar uchun bu xavfsiz — ular üchun
    router_admin hech narsani ushlamay, navbat router_search'ga o'tadi.
    """
    return (router_start, router_admin, router_search)


# ------------------------------------------------------------
# Render "Web Service" uchun mini HTTP server.
# Render bepul tarifi faqat portni tinglaydigan web-servislarni
# uyg'oq turishga ruxsat beradi. UptimeRobot shu manzilga
# (masalan: https://sizning-servis.onrender.com/) har 5 daqiqada
# so'rov yuborib, servisni uxlab qolishdan saqlaydi.
# ------------------------------------------------------------


async def handle_ping(request: web.Request) -> web.Response:
    return web.Response(text="Kinochi bot ishlayapti ✅")
...
async def run_web_server() -> None:
    ...
    await site.start()
    logger.info("HTTP server %s portda ishga tushdi (UptimeRobot uchun)", port)
...
async def main() -> None:
    ...
    try:
        ...
        # HTTP server va Telegram polling bir vaqtda ishlaydi
